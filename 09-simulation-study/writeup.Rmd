...
```{r parameter_list}
parameters <- list(n = 201, dist = "gamma", mean = 0, sd = 1, shape = 1.4, scale = 3, model = "moments_gamma", stat = "median", smoo = 0.3, R = 5000)
...
generate_data <- function(parameter) {
  if (parameter$dist == "normal") {
    ...
    parameter$data <- rgamma(n = parameters$n, shape = parameters$shape, scale = parameters$scale)
  }
  return(parameter)
}
...
capture_stat <- function(parameter) {
  ...
  if (parameter$dist == "normal" &
    ...
    if (parameter$stat == "median") {
      if (parameters$CI[1] < true.gamma.med &
          true.gamma.med < parameters$CI[2]) {
        count = 1
      } else {
        count = 0
      }
