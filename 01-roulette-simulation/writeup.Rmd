...
s1
plot(s1, type = "b") #Linear gains, exponential losses
plot(s1[1:10], type = "b")
...
There are also some simplifications in the stopping rule and next wagers, where we choose the minimum of the budget, maximum bet and number of plays. In a more realistic scenario, it is possible that we decide to dip into a extra cash fund for that very last play if these conditions are met. Another simplification that this simulation creates is the fact that we are assuming the player is will to play until they are bankrupt. In more realistic scenarios, it is possible that people my choose to stop playing once their budget has fallen to a certain amount. 
