...
1.  Compute analytically the probability that the Braves win the world
    ...
    In this method of trying to compute the probability that the Braves win the wrold series given a certain sequence of game locations, we use and modify the below code chunk provided to us. First, a data frame containing all the possible world series outcomes is read in. A home field indicator is established, which indicates what the series of playing home and away games would be. Variables for the probability of winning a single game, the advantage multiplier (10% advantage), the probability of winning a home game and an away game are created. Then we calculate the probability of each possible outcome in a series of 7 games, given a home field indicator and the subsequent advantage multiplier for that game location for the Braves. Another column containing the probability of winning each possible outcome is added to the initial data frame. The sum of the probability of winning or losing each game provides the probability of winning or losing the world series.
...
# Calculate the probability of each possible outcome for hfi1
apo[, p := NA_real_] # Initialize new column in apo to store prob
for(i in 1:nrow(apo)){
  prob_game <- rep(1, 7)
  for(j in 1:7){
    p_win <- ifelse(hfi1[j], pbh, pba) #Change hfi depending on what series is running
    prob_game[j] <- case_when(
        apo[i,j,with=FALSE] == "W" ~ p_win
      , apo[i,j,with=FALSE] == "L" ~ 1 - p_win
      , TRUE ~ 1
...
```
The probability of the Braves winning the world series given the home field advantage (More games were played at ATL than NYC) was `r AWinWithAd$V1[[1]]`, whlie the probability of them winning without the home field advantage was `r AWinWithoutAd$V1[[1]]`, given that the probability of winning any single game without the home field advantage is 0.55 and with the advantage is 0.55 plus a 10% advantage. 
...
for (i in 1:sims) {
    brave.win[i] <- world_series(hfi = hfi1, advantage_multiplier = 1.1)
}
